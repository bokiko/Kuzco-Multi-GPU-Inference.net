...
check_prerequisites() {
    ...
    if ! command -v nvidia-smi &> /dev/null; then
        ...
        print_colored $YELLOW "   Install with: sudo ubuntu-drivers autoinstall"
        exit 1
    fi
    ...
    if ! command -v docker &> /dev/null; then
        ...
        print_colored $YELLOW "   Install with: curl -fsSL https://get.docker.com | sh"
        exit 1
    fi
...
deploy_containers() {
    ...
    if [ $failed_deployments -eq 0 ]; then
        ...
        print_colored $YELLOW "   Check logs with: docker logs $name_prefix-0"
    fi
}
...
docker ps --filter "name=\$(jq -r '.container_prefix' "\$CONFIG_FILE" 2>/dev/null)" --format "table {{.Names}}\t{{.Status}}\t{{.RunningFor}}"
...
echo -e "\${CYAN}🔧 Restart installer: $(realpath "$0")\${NC}"
echo -e "\${CYAN}📊 Repository: https://github.com/bokiko/Multi-GPU-Inference.net\${NC}"
echo ""
EOF
